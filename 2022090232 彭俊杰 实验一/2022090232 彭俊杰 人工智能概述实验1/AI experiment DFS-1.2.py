...
version:V-1.1.2
file name:AI experiment DFS-1.2
"""
import operator
import copy
import time
...
openStack.append(startObject)  # 初始对象加入open表
...
if judge(startObject.Matrix, endMatrix):  # 判断是否有解
    while 1:
        if len(openStack) == 0:  # 判断在此深度下是否有解
            print("此最大深度下无解，请扩大最大深度")
            exit(1)
        else:
            if operator.eq(openStack[-1].Matrix, endMatrix):  # 判断open栈顶是否为目标
                closeList.append(openStack[-1])  # 压如close表
                endList = [endMatrix]  # 回溯列表
                father = closeList[-1].Father
                while father.Dn >= 1:
                    endList.append(father.Matrix)
                    father = father.Father
                print("变换成功，一共需要" + str(openStack[-1].Dn) + "次变换")
                print("初始状态：")
                ...
                if openStack[-1].Dn == 50:  # 限定最大深度，否则可能搜索很久
                    closeList.append(openStack[-1])
                    del openStack[-1]  # 直接进行弹栈
                else:
                    i0, j0 = find(openStack[-1].Matrix, 0)
                    operation = Operation(i0, j0, openStack[-1].camefrom)
                    nextObjectList = []  # 暂存列表
                    for i in operation:
                        # 对子对象因素赋值
                        nextMatrix = move(openStack[-1].Matrix, i)
                        nextObject = Object()
                        nextObject.Matrix = nextMatrix
                        nextObject.Dn = openStack[-1].Dn + 1
                        nextObject.camefrom = i
                        nextObject.Father = openStack[-1]
                        nextObjectList.append(nextObject)
                    # 将open表尾压如close表
                    closeList.append(openStack[-1])
                    del openStack[-1]
                    for i in nextObjectList:
                        openStack.append(i)  # 在表尾插入元素
else:
    print("无解")
end = time.time()
