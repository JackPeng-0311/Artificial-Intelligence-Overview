...
startObject.Fn = startObject.Dn + WrongNum(startObject.Matrix, endMatrix) + LengthNum(startObject.Matrix, endMatrix)  # 初始估值函数
openList.append(startObject)  # 初始对象加入open表
...
# A*算法
if judge(startObject.Matrix, endMatrix):
    while 1:
        if operator.eq(openList[0].Matrix, endMatrix):  # 找到目标
            closeList.append(openList[0])
            endList = []  # 回溯列表
            del openList[0]

            father = closeList[-1].Father
            endList.append(endMatrix)  # 从尾开始回溯
            while father.Dn >= 1:
                endList.append(father.Matrix)
                father = father.Father
            print("变换成功，一共需要" + str(openList[0].Dn) + "次变换")
            print("初始状态：")
            ...
            i0, j0 = find(openList[0].Matrix, 0)
            operation = Operation(i0, j0, openList[0].camefrom)  # 计算当前变换列表
            nextObjectList = []  # 用于暂时存储子对象
            for i in operation:
                # 对子对象的因素赋值
                nextMatrix = move(openList[0].Matrix, i)
                nextObject = Object()
                nextObject.Matrix = nextMatrix
                nextObject.Dn = openList[0].Dn + 1
                nextObject.Fn = nextObject.Dn + WrongNum(nextObject.Matrix, endMatrix)
                nextObject.camefrom = i
                nextObject.Father = openList[0]  # 当前矩阵的子矩阵
                nextObjectList.append(nextObject)
            closeList.append(openList[0])
            del openList[0]
            for i in nextObjectList:
                openList.append(i)
            openList.sort(key=lambda t: t.Fn)  # 排序，Fn最小的在表头
else:
    print("无解")
end = time.time()
